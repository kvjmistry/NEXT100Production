...
if [[ "${MODEL}" == "Livermore" ]]; then
    echo "Model is Livermore"
    sed -i "s#.*G4EmStandardPhysics_option4.*#/PhysicsList/RegisterPhysics G4EmLivermorePhysics#" ${INIT}
elif [[ "$model" == "Penelope" ]]; then
    echo "Model is Penelope"
    sed -i "s#.*G4EmStandardPhysics_option4.*#/PhysicsList/RegisterPhysics G4EmPenelopePhysics#" ${INIT}
else
    echo "Model is something else"
fi
...
python GetGammaTables.py NEXT100_Tl208_Port1a_nexus_${JOBID}.h5 NEXT100_Tl208_Port1a_GammaTables_${JOBID}.h5
